...
float FpsLimiter::end() {
	...
	unsigned int frameTicks = SDL_GetTicks() - _startTicks;
...
//Compute fps based on the last X executions
void FpsLimiter::calculateFPS() {
	//The number of frames to average
	static const int NUM_SAMPLES = 10;
	//Stores all the frametimes for each frame that we will average
	static unsigned int frameTimes[NUM_SAMPLES];
	//The current frame we are on. Remember, it is only assigned the first time it's executed because it's static
	...
	} else { //This only will happen the fist time it enters into this method
		_fps = 60.0f;
	}
}
